...
int main(int argc, char **argv)
{
    ...
    while (cv::waitKey(10) != 'q')
    {
        ...
        /* Fitness 의 합이 가장 작은 Swarm 구하기 */
        double compare_value = 0;
        double best_swarm_index = 0;
        for (int i = 0; i < swarms_total_fitness.size(); i++)
        {
            if (i == 0)
            {
                best_swarm_index = i;
                compare_value = swarms_total_fitness[i];
            }
            else if (swarms_total_fitness[i] < compare_value)
            {
                best_swarm_index = i;
                compare_value = swarms_total_fitness[i];
            }

            std::cout << swarms_total_fitness[i] << "\t\t";
        }
        std::cout << std::endl;
        std::cout << "Best Swarms: " << best_swarm_index << "\t\tFitness : " << compare_value << std::endl;
